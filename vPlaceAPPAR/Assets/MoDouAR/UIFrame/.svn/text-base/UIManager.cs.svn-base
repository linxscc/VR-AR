...
namespace MoDouAR
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        /// </summary>
        private Stack<WindowsBase> stackCurrentUIForms;
        ...
        public void CloseUIForm(Wind wind)
        {
            WindowsBase baseUiForm;                          //窗体基类

            //参数检查
            if (string.IsNullOrEmpty(wind.name)) return;
            //“所有UI窗体”集合中，如果没有记录，则直接返回
            allUIForms.TryGetValue(wind.name, out baseUiForm);
            ...
            switch (baseUiForm.CurrentUIType.uiFormShowMode)
            {
                ...
                case UIFormShowMode.HideOther:
                    //隐藏其他窗体关闭
                    ExitUIFormsAndDisplayOther(wind.name);
                    break;
        ...
        public void ShowUIForm(Wind wind)
        {
            if (allUIForms.ContainsKey(wind.name))
            {
                WindowsBase baseUI = allUIForms[wind.name].GetComponent<WindowsBase>();
                if (baseUI.CurrentUIType.isClearStack)
                    ClearStackArray();
                switch (baseUI.CurrentUIType.uiFormShowMode)
                {
                    case UIFormShowMode.Normal:
                        LoadUIToCurrentCache(wind.name);
                        break;
                    case UIFormShowMode.ReverseChange:
                        PushUIFormToStack(wind.name);
                        break;
                    case UIFormShowMode.HideOther:
                        EnterUIFormsAndHideOther(wind.name);
                        break;
                    default:
                        break;
                }
        ...
        /// <returns></returns>
        public GameObject CreatWindows(Wind wind)
        {
            WindowsBase windows=null;
            GameObject wndObj = GameObject.Instantiate(Resources.Load(wind.path)) as GameObject;
            if (wndObj == null) return null;
            wndObj.name = wind.name;
            ...
            allUIForms.Add(wind.name, windows);
            return wndObj;
        }
        /// <summary>
        /// 把当前窗体加载到“当前窗体”集合中
