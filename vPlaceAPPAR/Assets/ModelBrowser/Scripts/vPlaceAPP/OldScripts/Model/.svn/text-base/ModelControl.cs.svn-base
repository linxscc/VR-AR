...
namespace vPlace_zpc
{
    public class ModelControl : MonoBehaviour
    {
        ...
        public void BackHome()
        {
            ...
            model.transform.DOLocalMove(ProjectConstDefine.labelDataList.transform.Position, 0.2f);
            model.transform.DORotate(ProjectConstDefine.labelDataList.transform.Rotation, 0.2f);
            model.transform.DOScale(ProjectConstDefine.labelDataList.transform.Scale, .2f);
            //模型父级归零，重置
        ...
        public void LoadModel(GameObject obj, LabelDataList labelDataList)
        {
            try
            {
                ...
                if (ProjectConstDefine.hasConfig)
                    HandleOnInitModel(obj, ProjectConstDefine.labelDataList);
                else
        ...
        {
            if (obj.layer != 9)
                StartSceneControl.Singleton.startScene.dirLight.GetComponent<Light>().color = Color.white;
            else
                StartSceneControl.Singleton.startScene.dirLight.GetComponent<Light>().color = Color.black;
        }
        ...
        public void CameraReset()
        {
            ...
            {
                Global.camera.transform.localPosition = Vector3.zero;
                Global.camera.transform.localRotation = Quaternion.identity;
