...
namespace ModelViewerProject.Label3D
{
    public class Label3DController : MonoBehaviour
    {
        ...
        public LabelDataList DataList
        {
            get
            {
                List<LabelData> datas = labels.Select(l => new LabelData()
                {
                    title = l.Title,
                ...
                return new LabelDataList() { transform = transform.Serialization(myName, localName), animationType = (int)animaiomType, controlType = (int)controlType, list = datas };
            }
        }

        //Init by Editor Window
        public void OnInit(Transform modRoot, string myName,string localName,AnimaiomType type, ControlType conType)
        {
