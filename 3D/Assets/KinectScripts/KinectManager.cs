...
/* 2017.04.21 补充注释,夕阳如风
 
   1. 如果你要看深度图像，你可以开启‘Compute User Map’ 和 ‘Display User Map’参数；
   1.如果你想看色彩图像，开启‘Compute Color Map’ 和 ‘Display Color Map’参数；
   1.看骨骼追踪效果可以开启‘Display Skeleton Lines’参数
   1.使计算用户图像和图像显示用户的参数,如果你想看到user-depth图像在屏幕上。
   1.使“计算彩色图像”和“显示彩色图像”参数,如果你想看彩色摄像机图像在屏幕上。
   1.使显示骨架线的参数,如果你想看看Kinect跟踪user-depth图像上的骨架。
...
#region 新添事件 Time:2017.04.18
/// <summary>
/// 事件触发
/// </summary>
[Serializable]
public class TimeEventWave_T
{
    [Serializable]
    public class EventWave : UnityEvent
    {
    }
    [SerializeField]
    public EventWave m_EventWave = new EventWave();
}
#endregion

/// <summary>
/// [T:KinectManager是主要的Kinect相关组件，用于传感器和Unity应用程序之间的通信。]KinectManager is the main Kinect-related component,  used for communication between the sensor and the Unity application.
/// </summary>
public class KinectManager : MonoBehaviour
{
    #region 新添初始化KinectManager数据属性 Time:2017.05.19
    /// <summary>
    /// 初始化该类 属性
    /// </summary>
    private void InitKinectManager()
    {
        computeUserMap = KinectManager.UserMapType.None;
        //是否使用 彩色摄像机图像
        //kmanager.computeColorMap = true;
        //是否显示用户
        //kmanager.displayUserMap = true;
        displayColorMap = true;
        displaySkeletonLines = true;
        showTrackedUsersOnly = true;
        detectClosestUser = true;
        ignoreInferredJoints = true;
        useBoneOrientationConstraints = true;
    }
    #endregion
    ...
    /// 传感器有多高（以米为单位）。
    /// </summary>
    [Tooltip("传感器有多高（以米为单位）。")]
    public float sensorHeight = 1.0f;
    /// <summary>
    /// Kinect仰角（以度为单位）。 可能是积极或消极的。
    /// </summary>
    [Tooltip("Kinect仰角（以度为单位）。 可能是积极或消极的。")]
    ...
    /// <summary>
    /// 是否自动设置传感器的高度和角度。 用户必须保持在传感器的前方，以便自动检测工作
    /// </summary>
    [Tooltip("是否自动设置传感器的高度和角度。 用户必须保持在传感器的前方，以便自动检测工作")]
    ...
    /// 是否在屏幕上显示用户。
    /// </summary>
    [Tooltip("是否在屏幕上显示用户地图。")]
    public bool displayUserMap = false;
    /// <summary>
    /// 是否在屏幕上显示彩色摄像机图像。
    /// </summary>
    [Tooltip("是否在屏幕上显示彩色摄像机图像。")]
    ...
    /// <summary>
    /// [T:如果百分比为零，则在内部进行计算，以与深度图像的所选宽度和高度相匹配] if percent is zero, it is calculated internally to match the selected width and height of the depth image 
    /// <summary>
    [Tooltip("屏幕上的深度和彩色图像宽度，作为相机宽度的百分比。 图像高度根据宽度计算。")]
    public float DisplayMapsWidthPercent = 20f;

    /// <summary>
    /// 是否使用多源阅读器，如果有可用（仅K2功能）。
    /// </summary>
    [Tooltip("[T:是否使用多源阅读器，如果有可用（仅K2功能）。] Whether to to use the multi-source reader, if one is available (K2-only feature).")]
    public bool useMultiSourceReader = false;

    /// <summary>
    ///  [T:公共Bool确定是否使用传感器的音频源，如果可用] Public Bool to determine whether to use sensor's audio source, if available
    /// </summary>
    [Tooltip("公共Bool确定是否使用传感器的音频源，如果可用")]
    public bool useAudioSource = false;

    /// <summary>
    /// 与用户的最小距离，以便被考虑用于骨架数据处理。
    /// </summary>
    [Tooltip("[T:与用户的最小距离，以便被考虑用于骨架数据处理。]Minimum distance to a user, in order to be considered for skeleton data processing.")]
    public float minUserDistance = 0.5f;
    /// <summary>
    /// 到用户的最大距离，以便被考虑用于骨架数据处理。 值0表示无最大距离限制。
    /// </summary>
    [Tooltip("[T:到用户的最大距离，以便被考虑用于骨架数据处理。 值0表示无最大距离限制。]Maximum distance to a user, in order to be considered for skeleton data processing. Value of 0 means no maximum distance limitation.")]
    public float maxUserDistance = 0f;
    /// <summary>
    /// 与用户最大距离或最远距离，以便被考虑用于骨架数据处理。 值0表示没有左/右距离限制。
    /// </summary>
    [Tooltip("[T:与用户最大距离或最远距离，以便被考虑用于骨架数据处理。 值0表示没有左/右距离限制。]Maximum left or right distance to a user, in order to be considered for skeleton data processing. Value of 0 means no left/right distance limitation.")]
    public float maxLeftRightDistance = 0f;

    /// <summary>
    /// 最多可以同时跟踪的用户数。
    /// </summary>
    [Tooltip("[T:最多可以同时跟踪的用户数。]Maximum number of users, which may be tracked simultaneously.")]
    public int maxTrackedUsers = 6;

    /// <summary>
    /// 是否仅将所跟踪的用户显示在允许的距离内，或所有用户（更高的fps）。
    /// </summary>
    [Tooltip("[T:是否仅将所跟踪的用户显示在允许的距离内，或所有用户（更高的fps）。]Whether to display the tracked users within the allowed distance only, or all users (higher fps).")]
    public bool showTrackedUsersOnly = true;
    /// <summary>
    /// 是否首先检测最接近的用户？
    /// </summary>
    [Tooltip("[T:是否首先检测最接近的用户？]Whether to detect only the closest user first or not.")]
    public bool detectClosestUser = true;

    /// <summary>
    /// 是否仅使用跟踪关节（并忽略推测的关节）。
    /// </summary>
    [Tooltip("[T:是否仅使用跟踪关节（并忽略推测的关节）。]Whether to utilize only the tracked joints (and ignore the inferred ones) or not.")]
    public bool ignoreInferredJoints = false;

    /// <summary>
    /// 是否忽略关节的Z坐标（即在2D场景中使用它们）。
    /// </summary>
    [Tooltip("[T:是否忽略关节的Z坐标（即在2D场景中使用它们）。]Whether to ignore the Z-coordinates of the joints (i.e. use them in 2D-scenes) or not.")]
    public bool ignoreZCoordinates = false;

    /// <summary>
    /// 是否更新LateUpdate（）中的AvatarController，而不是Update（）。 需要Mocap-Mecanim混合。
    /// </summary>
    [Tooltip("[T:是否更新LateUpdate（）中的AvatarController，而不是Update（）。 需要Mocap-Mecanim混合。]Whether to update the AvatarControllers in LateUpdate(), instead of in Update(). Needed for Mocap-Mecanim blending.")]
    ...
    /// 一组关节平滑参数。
    /// </summary>
    [Tooltip("[T:一组关节平滑参数]Set of joint smoothing parameters.")]
    public Smoothing smoothing = Smoothing.Default;
    /// <summary>
    /// 是否应用骨骼方向约束。
    /// </summary>
    [Tooltip("[T:是否应用骨骼方向约束。]Whether to apply the bone orientation constraints.")]
    ...
    /// 是否允许检测身体转身。
    /// </summary>
    [Tooltip("[T:是否允许检测身体转身。]Whether to allow detection of body turn arounds or not.")]
    public bool allowTurnArounds = false;

    public enum AllowedRotations : int { None = 0, Default = 1, All = 2 }

    /// <summary>
    /// 允许手腕和手旋转。 无 - 无手旋转，默认 - 手旋转允许，除了扭曲，全部 - 所有旋转是允许的。
    /// </summary>
    [Tooltip("[T:允许手腕和手旋转。 无 - 无手旋转，默认 - 手旋转允许，除了扭曲，全部 - 所有旋转是允许的。]Allowed wrist and hand rotations. None - no hand rotations, Default - hand rotations are allowed except the twists, All - all rotations are allowed.")]
    public AllowedRotations allowedHandRotations = AllowedRotations.Default;
    /// <summary>
    /// 丢失用户被删除之前等待时间（秒）。 这是为了防止零星的用户切换。
    /// </summary>
    [Tooltip("[T:丢失用户被删除之前等待时间（秒）。 这是为了防止零星的用户切换。]Wait time in seconds, before a lost user gets removed. This is to prevent sporadical user switches.")]
    public float waitTimeBeforeRemove = 1f;
    /// <summary>
    /// 在场景中受控化身的列表。 如果列表为空，则可以在启动时检测到可用的化身控制器。
    /// </summary>
    [Tooltip("[T:在场景中受控化身的列表。 如果列表为空，则可以在启动时检测到可用的化身控制器。]List of the controlled avatars in the scene. If the list is empty, the available avatar controllers will be detected at start up.")]
    public List<AvatarController> avatarControllers = new List<AvatarController>();

    /// <summary>
    /// 打开相应播放器的跟踪所需的校准姿势。
    /// </summary>
    [Tooltip("[T:打开相应播放器的跟踪所需的校准姿势。]Calibration pose required to turn on the tracking of respective player.")]
    public KinectGestures.Gestures playerCalibrationPose;
    /// <summary>
    /// 每个玩家要检测的手势列表。
    /// </summary>
    [Tooltip("[T:每个玩家要检测的手势列表。]List of Gestures to be detected for each player.")]
    public List<KinectGestures.Gestures> playerCommonGestures = new List<KinectGestures.Gestures>();
    /// <summary>
    /// 手势检测之间的最短时间（以秒为单位）。
    /// </summary>
    [Tooltip("[T:手势检测之间的最短时间（以秒为单位）。]Minimum time between gesture detections (in seconds).")]
    public float minTimeBetweenGestures = 0.7f;
    /// <summary>
    /// 手势管理器，用于检测程式化Kinect手势。
    /// </summary>
    [Tooltip("[T:手势管理器，用于检测程式化Kinect手势。]Gesture manager, used to detect programmatic Kinect gestures.")]
    public KinectGestures gestureManager;
    /// <summary>
    /// 可用手势监听器的列表。 他们必须实现KinectGestures.GestureListenerInterface。 如果列表为空，启动时将检测到可用的手势监听器。
    /// </summary>
    [Tooltip("[T:可用手势监听器的列表。 他们必须实现KinectGestures.GestureListenerInterface。 如果列表为空，启动时将检测到可用的手势监听器。]List of the available gesture listeners. They must implement KinectGestures.GestureListenerInterface. If the list is empty, the available gesture listeners will be detected at start up.")]
    ...
    /// <summary>
    /// GUI - 文本显示当前跟踪的手势的调试消息。
    /// </summary>
    [Tooltip("GUI-Text to display debug messages for the currently tracked gestures.")]
    public GUIText gesturesDebugText;

    /// <summary>
    /// [T:Bool来跟踪Kinect是否已被初始化] Bool to keep track of whether Kinect has been initialized
    /// </summary>
    private bool kinectInitialized = false;

    /// <summary>
    /// [T:KinectManager的单例实例]The singleton instance of KinectManager
    /// </summary>
    private static KinectManager instance = null;

    /// <summary>
    ///  [T:可用传感器接口]available sensor interfaces
    /// </summary>
    private List<DepthSensorInterface> sensorInterfaces = null;
    /// <summary>
    ///  [T:主要的传感器数据结构] primary SensorData structure
    /// </summary>
    private KinectInterop.SensorData sensorData = null;

    // [T:深度和用户图像] Depth and user maps
    ...
    /// <summary>
    ///  [T: 用户是否受到数量或距离的限制] Whether the users are limited by number or distance
    /// </summary>
    private bool bLimitedUsers = false;

    /// <summary>
    ///   [T:主（最初或最近）用户ID] Primary (first or closest) user ID
    /// </summary>
    private Int64 liPrimaryUserId = 0;

    /// <summary>
    /// [T: Kinect到世界矩阵]    Kinect to world matrix
    /// </summary>
    private Matrix4x4 kinectToWorld = Matrix4x4.zero;
    //private Matrix4x4 mOrient = Matrix4x4.zero;

    /// <summary>
    // [T:每个玩家的校准手势数据] Calibration gesture data for each player
    /// </summary>
    ...
    /// <summary>
    /// [T:手势数据和参数] gestures data and parameters
    /// </summary>
    ...
    /// <summary>
    ///  [T:手势听众名单。 他们必须实现KinectGestures.GestureListenerInterface] List of Gesture Listeners. They must implement KinectGestures.GestureListenerInterface
    /// </summary>
    //public List<KinectGestures.GestureListenerInterface> gestureListenerInts;

    // [T:身体过滤器实例] Body filter instances
    ...
    // [T:背景kinect线程] background kinect thread
    private System.Threading.Thread kinectReaderThread = null;
    private bool kinectReaderRunning = false;


    /// <summary>
    /// [T:获取单个KinectManager实例。] Gets the single KinectManager instance.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定传感器和KinectManager组件是否被初始化并可以使用。]Determines if the sensor and KinectManager-component are initialized and ready to use.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取传感器数据结构（此结构不应被修改，只能在内部使用）。] Gets the sensor data structure (this structure should not be modified and must be used only internally).
    /// </summary>
    ...
    /// <summary>
    /// [T:获得给定关节的父级关节 ] Gets the parent joint of the given joint.
    /// </summary>
    ...
    /// <summary>
    /// [T:获得给定联合的下一个关节。] Gets the next joint of the given joint.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取传感器返回的彩色图像的宽度。]Gets the width of the color image, returned by the sensor.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取传感器返回的彩色图像的高度。]Gets the height of the color image, returned by the sensor.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取传感器返回的深度图像的宽度。]Gets the width of the depth image, returned by the sensor.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取传感器返回的深度图像的高度。]Gets the height of the depth image, returned by the sensor.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定传感器当前是否检测到至少一个用户] Determines whether at least one user is currently detected by the sensor
    /// </summary>
    ...
    /// <summary>
    /// [T:确定具有指定userId的用户是否在跟踪用户的列表中。] Determines whether the user with the specified userId is in the list of tracked users or not.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取所有当前跟踪的用户的ID。]Gets IDs of all currently tracked users.
    /// </summary>
    ...
    /// <summary>
    /// [T:通过指定的用户索引获取用户ID。]Gets the user ID by the specified user index.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取跟踪的身体指数列表。] Gets the list of tracked body indices.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取主用户的用户ID（第一个或最近一个），如果没有检测到用户，则为0。] Gets the UserID of the primary user (the first or the closest one), or 0 if no user is detected.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取身体索引[0-5]，如果选择了单个身体来显示在用户地图上，如果显示所有身体，则为-1。] Gets the body index [0-5], if there is single body selected to be displayed on the user map, or -1 if all bodies are displayed.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取用户身体数据（仅供内部使用）。] Gets the user body data (for internal purposes only).
    /// </summary>
    ...
    /// <summary>
    /// [T:获取kinect到世界矩阵。] Gets the kinect to world matrix.
    /// </summary>
    /// <returns>[T:kinect到世界矩阵。] The kinect to world matrix.</returns>
    ...
    /// <summary>
    /// [T:以米为单位获取相对于传感器的用户位置。] Gets the user position, relative to the sensor, in meters.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取用户方向。] Gets the user orientation.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取关节的跟踪状态。] Gets the tracking state of the joint.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定是否正在跟踪指定用户的给定关节。] Determines whether the given joint of the specified user is being tracked.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户在Kinect坐标系中的关节位置，单位为米。] Gets the joint position of the specified user, in Kinect coordinate system, in meters.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的关节位置（以米为单位）。] Gets the joint position of the specified user, in meters.
    /// </summary>
    /// <param name="userId">[T: 用户ID] User ID</param>
    /// <param name="joint">[T: 关节索引] Joint index</param>
    /// <returns>[T: 返回关节位置]The joint position.</returns>
    ...
    /// <summary>
    /// [T:获取指定用户相对于其父节点的关节方向。] Gets the joint direction of the specified user, relative to its parent joint.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的给定关节之间的方向。] Gets the direction between the given joints of the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的关节方向。] Gets the joint orientation of the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取给定关节的3D叠加位置在深度图像上。] Gets the 3d overlay position of the given joint over the depth-image.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取给定关节在颜色图像上的3D叠加位置。] Gets the 3d overlay position of the given joint over the color-image.
    /// </summary>
    ...
    /// <summary>
    /// [T:在深度图像上获取关节位置] Gets the joint position on the depth map texture.
    /// </summary>
    ...
    /// <summary>
    /// [T:在彩色图像上获取关节位置] Gets the joint position on the color map texture.
    /// </summary>
    /// <returns>The joint position in texture coordinates.</returns>
    /// <param name="userId">User ID</param>
    /// <param name="joint">Joint index</param>
    ...
    /// <summary>
    /// [T:确定给定用户是否转身。] Determines whether the given user is turned around or not.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定指定用户的左手置信度是否高。] Determines whether the left hand confidence is high for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定指定用户的右手置信度是否高。] Determines whether the right hand confidence is high for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的左手 手势状态。] Gets the left hand state for the specified user.
    /// </summary>
    /// <returns>[T:返回 左手的手势状态] The left hand state.</returns>
    /// <param name="userId">[T: 用户ID ] User ID</param>
    ...
    /// <summary>
    /// [T:获取指定用户的右手状态。]Gets the right hand state for the specified user.
    /// </summary>
    /// <param name="userId">[T: 用户ID]User ID</param>
    /// <returns>[T:返回 右手的状态] The right hand state.</returns>
    ...
    /// <summary>
    /// [T:获取指定用户的左手交互框。] Gets the left hand interaction box for the specified user.
    /// </summary>
    /// <returns><c>true</c>, [T:如果得到了左手交互框为true，否则为false 。] if left hand interaction box was gotten, <c>false</c> otherwise.</returns>
    /// <param name="userId">User ID</param>
    /// <param name="leftBotBack">[T:包含左，底和后坐标的矢量，以米为单位] Vector containing the left, bottom and back coordinates, in meters</param>
    /// <param name="rightTopFront">[T:矢量包含右，顶和前坐标，以米为单位] Vector containing the right, top and front coordinates, in meters</param>
    /// <param name="bValidBox">[T:如果设置为 true ，则先前设置的坐标有效] If set to <c>true</c>, the previously set coordinates are valid</param>
    ...
    /// <summary>
    /// [T:获取指定用户的右手交互框。] Gets the right hand interaction box for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:如果ComputeUserMap为true，则获取指定像素的深度值。 ] Gets the depth value for the specified pixel, if ComputeUserMap is true.
    /// </summary>
    ...
    /// <summary>
    /// [T:返回深度图点的空间坐标，如果传感器未初始化，则返回Vector3.zero] Returns the space coordinates of a depth-map point, or Vector3.zero if the sensor is not initialized
    /// </summary>
    ...
    /// <summary>
    /// [T:将深度框映射到空间坐标。] Maps the depth frame to space coordinates.
    /// </summary>
    ...
    /// <summary>
    /// [T:返回空间点的深度图坐标，如果Kinect未初始化，则返回Vector2.zero] Returns the depth-map coordinates of a space point, or Vector2.zero if Kinect is not initialized
    /// </summary>
    ...
    /// <summary>
    /// [T:返回深度点的颜色 - 地图坐标。] Returns the color-map coordinates of a depth point.
    /// </summary>
    ...
    /// <summary>
    /// [T:将深度框映射到颜色坐标。] Maps the depth frame to color coordinates.
    /// </summary>
    /// <returns><c>true</c> on success, <c>false</c> otherwise.</returns>
    ...
    /// <summary>
    /// [T:将颜色框映射到深度坐标。] Maps the color frame to depth coordinates.
    /// </summary>
    /// <returns><c>true</c> on success, <c>false</c> otherwise.</returns>
    ...
    /// <summary>
    /// [T:返回一个色点的深度 - 地图坐标。] Returns the depth-map coordinates of a color point.
    /// </summary>
    ...
    /// <summary>
    /// [T:删除所有当前检测到的用户，允许新的用户检测过程启动。] Removes all currently detected users, allowing new user-detection process to start.
    /// </summary>
    ...
    /// <summary>
    /// [T:重置Kinect数据过滤器。] Resets the Kinect data filters.
    /// </summary>
    ...
    /// <summary>
    /// [T:将手势添加到指定用户检测到的手势列表中。]Adds a gesture to the list of detected gestures for the specified user.
    /// </summary>
    /// <param name="UserId">User ID</param>
    ...
    /// <summary>
    /// [T:重置指定用户的给定手势的手势状态。] Resets the gesture state for the given gesture of the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:重置指定用户的所有手势的手势状态。] Resets the gesture states for all gestures of the specified user.
    /// </summary>
    /// <param name="UserId">User ID</param>
    ...
    /// <summary>
    /// [T:删除指定用户的所有手势。] Deletes all gestures for the specified user.
    /// </summary>
    /// <param name="UserId">User ID</param>
    ...
    /// <summary>
    /// [T:获取指定用户的手势列表。] Gets the list of gestures for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的手势计数。] Gets the gestures count for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:以给定用户的指定索引获取手势。] Gets the gesture at the specified index for the given user.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定给定手势是否在指定用户的手势列表中。] Determines whether the given gesture is in the list of gestures for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定指定用户的给定手势是否完整。] Determines whether the given gesture for the specified user is complete.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定指定用户的给定手势是否被取消。] Determines whether the given gesture for the specified user is canceled.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的给定手势的进度（范围[0，1]）。] Gets the progress (in range [0, 1]) of the given gesture for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取指定用户的给定手势的标准化屏幕位置。] Gets the normalized screen position of the given gesture for the specified user.
    /// </summary>
    ...
    /// <summary>
    /// [T:获取body框架作为一个csv行，或返回空字符串，如果没有新的body框架。] Gets the body frame as one csv line, or returns empty string if there is no new body frame.
    /// </summary>
    ...
    /// <summary>
    /// [T:确定播放模式是否启用。] Determines whether the play mode is enabled or not.
    /// </summary>
    ...
    /// <summary>
    /// [T:启用或置换播放模式。] Enables or displables the play mode.
    /// </summary>
    ...
    /// <summary>
    /// [T:从给定的csv行设置正文框架。] Sets the body frame from the given csv line.
    /// </summary>
    /// <returns><c>true</c> on success, <c>false</c> otherwise.</returns>
    ...
    // [T:KinectManager的内部方法] KinectManager's Internal Methods


    void Awake()
    {
        #region 初始化新添  Time:2017.05.19
        InitKinectManager();
        #endregion
        ...
        try
        {
            ...
            // [T:初始化可用的传感器接口] init the available sensor interfaces
    ...
    void StartKinect()
    {
        try
        {
            ...
            // [T:启用或禁用获取高度和角度信息] enable or disable getting height and angle info
            sensorData.hintHeightAngle = (autoHeightAngle != AutoHeightAngle.DontUse);

            // [T:创建变换矩阵 - kinect到世界] create the transform matrix - kinect to world
        ...
        // [T: 设置这个类的单列] set the singleton instance
        instance = this;

        // [T: init骨架结构] init skeleton structures
        ...
        {
            // [T:初始化深度和标签贴图相关的东西] Initialize depth & label map related stuff
        ...
        // [T:尝试自动使用场景中可用的头像控制器] try to automatically use the available avatar controllers in the scene
        ...
        // [T:设置手势管理器，如果还没有设置] set up the gesture manager, if not already set
        ...
        // [T:尝试自动使用场景中可用的手势监听器] try to automatically use the available gesture listeners in the scene
    ...
    void OnGUI()
    {
        if (kinectInitialized)
        {
               ...
               (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
            {
                ...
                {
                    // [T:得到主相机矩形] get the main camera rectangle
                    Rect cameraRect = Camera.main != null ? Camera.main.pixelRect : new Rect(0, 0, Screen.width, Screen.height);

                    // [T:如果需要，以百分比计算图像宽度和高度] calculate map width and height in percent, if needed
    ...
    // [T:更新Kinect流和结构] updates Kinect streams and structures
    private void UpdateKinectStreams()
    {
        if (kinectInitialized)
        {
            // [T:检查用户限制并更新传感器数据] check user limits and update sensor data
            ...
            {
                // [T:过滤跟踪的关节位置] filter the tracked joint positions
    ...
    // [T:背景kinect线程程序] background kinect thread procedure
    ...
    // [T:来自Kinect流的流程数据] process data from Kinect streams
    ...
    private void ProcessBodyFrameData()
    {
        ...
        for (int i = 0; i < sensorData.bodyCount; i++)
        {
               ...
               (maxTrackedUsers < 0 || trackedUsers < maxTrackedUsers))
            {
                ...
                // [T:将Kinect坐标转换为世界坐标] convert Kinect positions to world positions
                ...
                // [T:计算身体关节的世界取向] calculate world orientations of the body joints
    ...
    // [T:计算身体数据中的特殊方向和其他有用数据] calculates special directions and other useful data out of the body data
                           ...
                           && (joint != (int)KinectInterop.JointType.HandRight))
                        {
                            ...
                            if (thumbData.trackingState != KinectInterop.TrackingState.NotTracked)
                            {
                                ...
                                if (rightDir != Vector3.zero && fwdDir != Vector3.zero)
                                {
                                    ...
                                    //									// [T:修复无效的手腕旋转] fix invalid wrist rotation
