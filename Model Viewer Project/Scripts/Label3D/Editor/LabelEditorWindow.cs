    ...
    public class LabelEditorWindow : EditorWindow
    {
        ...
        void OnSaveLabelText()
        {
            ...
            if (path.Length != 0)
            {
                ...
                labelController.OnInit(sceneObj.transform, prefabName ,animaiomType, controlType);
        ...
        void OnLoadLabelText()
        {
            ...
            if (path.Length != 0)
            {
                ...
                var dataList = JsonFx.Json.JsonReader.Deserialize<LabelDataList>(_InternalDataCodec.LoadString(path));
        ...
        void OnSaveAssetBundle()
        {
            ...
            if (path.Length != 0)
            {
                ...
                assetBundle[0].assetBundleName = srcMeshObj.name + ".unity3d";
                //Build Assetbundle
                //BuildPipeline.BuildAssetBundle(null, toinclude.ToArray(), path, BuildAssetBundleOptions.CollectDependencies);
                BuildPipeline.BuildAssetBundles(path, assetBundle, BuildAssetBundleOptions.None, BuildTarget.StandaloneWindows64);
